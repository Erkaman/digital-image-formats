...
\begin{refsection}
  ...
  But parsing the data from the image descriptor is not as easy. Let
  us consider how to parse the image origin data. If the 5:th bit is
  one that means that first part of the destination is
  ``bottom''. Else it must be ``top''. The fifth bit has be value
  $1 \ShiftLeft 5$, thus we can get the value of the fifth bit of the
  image descriptor by the operation $(1 \ShiftLeft 5) \BitAnd
  imageDescriptor$.

  In the same way the value of the 4:th bit is $(1 \ShiftLeft 4)
  \BitAnd imageDescriptor$. If the 4:th bit is toggled then second
  part of the destination string must ``right'', else it must be
  ``left''.

  We summarize this whole disscussion in algorithm
  \ref{alg:tga-dest-str}.

  \todo{fix strings description}
  \begin{algorithm}[H]
    \caption{Getting the destination string of image descriptor.}
    \label{alg:tga-dest-str}
    \begin{algorithmic}[1]
      \Let{$destStr$}{``''}
      \If{$(1 \ShiftLeft 5) \BitAnd imageDescriptor$}
        \Inc{$destStr$}{``Top ''}
      \Else
        \Inc{$destStr$}{``Bottom ''}
      \EndIf
      \If{$(1 \ShiftLeft 4) \BitAnd imageDescriptor$}
        \Inc{$destStr$}{``right''}
      \Else
        \Inc{$destStr$}{``left''}
      \EndIf
...
%  \subsection*{The getBits function}
