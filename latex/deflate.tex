...
The length codes are a bit tricky to use, though. Please observe
...
will be represented by the 7-bit Huffman Code \bin{0000000}, the
literal code 142 will on the other hand be represented by the 8-bit
Huffman code \bin{10111110} and the length code 258 will be
represented by the 7-bit Huffman code \bin{0000010}. The reader may
...
The two codes 286 and 287 will according to the \deflate specification
...
first minimum value will always be 0. $\var{minCodes[2]}$ will on the
  ...
  because this is the sum of the former minimum code and the frequency
  of the former code lengths left shifted by one. And if you inspect
  the tree this makes perfect since; the minimum values for codes of
  the code length $2$, the only code \texttt{d}, is, indeed
  \bin{10}. And similarly,
  ...
  Regrettably, we cannot give a reasonable explanation why this step
  works, since we cannot understand it. However, since this method was
  reccomended directly by the Deflate specification, we can be pretty
  sure that it will always work.
