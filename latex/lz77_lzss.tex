...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Repeatn{$\var{tokenLength}$}
      \Let{$\var{decompressed[length]}$}{$\var{decompressed[length - tokenOffet]}$}
    \EndRepeatn
  \end{algorithmic}
\end{algorithm}
...
So for a window size of $w$ the bit size of the number specifying
offsets must be.
...
an ASCII code of $65$, would be represented by the bit pattern \bin{1\
  0100\ 0001}. Match tokens are represented by a toggled bit followed
