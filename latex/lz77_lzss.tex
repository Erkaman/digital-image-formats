...
%  \lzonestate{she{\spc}sells{\spc}se}{{\spc}shel}{5}{3}{a}
}
...
It is easy to see why LZ77 is able to perform compression working in
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \linecomment{$length$ is the length of the so far decompressed data. }
    \Repeatn{$tokenLength}$
      \Let{$decompressed[length]$}{$decompressed[length -
        tokenOffet]$}
      \Let{$length$}{$length + 1$}
    \EndRepeatn
  \end{algorithmic}
