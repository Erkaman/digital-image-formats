...
The entire addition operation is defined in table
...
This operation may seem complex, but it is in fact easy to
...
In the following example, we'll use the arbitrarily chosen generator
...
$x^3 + x^2 + x^0$ by the hexadecimal number \hex{d}($=1101$). However,
...
example have written that last polynomial as \hex{5}($=0101$), because
...
would have written it as \hex{9}($=101$), because they first of all
...
Subtraction is the same thing as the bitwise xor operator in CRC
...
So, the two division operations on the two separate bytes is
...
pattern $1111\ 1111$(=\hex{FF)}, you can make sure that you only get
the 8 lowest bits.
...
are instead of \hex{FF}. My main advise is to always read the
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{crc32}{$bytes, G(x)$}
    \Let{result}{$0$}

    \ForEach{$byte$}{$bytes$}
    \Let{$result$}{$result \BitXor (byte \ShiftLeft 24)$}

    \Repeatn{$8$}
    \If{$result \BitAnd \hex{80000000}$}
    \Let{$result$}{$(result \ShiftLeft 1) \BitXor G(x)$}
    \Else
    \Let{$result$}{$result \ShiftLeft 1$}
    \EndIf
...
Another checksum is \adlerchk. It is for example used to verify data
integrity in the ZLIB format\cite{feldspar:_explan_deflat_algor}. The
ZLIB format is of interest to us, because that is the container format
used to contain the color data in PNG images. But we will for only
discuss \adlerchk, the checksum in the ZLIB format.
...
data. Once it has, the 16 highest bits of the resulting checksum are
set to $b$ and the 16 lowest bits are set to $a$. Hence, the \adlerchk
checksum is always a 32-bit number, as is indicated by its name. In
other words, for some data that has a length of $n$ bytes the checksum
is calculated as follows:\footnote{Go back to exercise \ref{bit-equiv}
  if you don't understand the conversion in the last line. }
...
\begin{equation*}
  ...
  \begin{array}{l}
    ...
    \text{adler32} = (b \ShiftLeft 16) \BitOr a = (b \cdot 65536) + a \\[\between]
  \end{array}
\end{equation*}
...
\begin{algorithm}[H]
  ...
  \label{alg:adler32}
  \begin{algorithmic}[1]
    \Procedure{adler32}{$data$}

    \Let{$a$}{$1$}
    \Let{$b$}{$0$}
    \ForEach{$byte$}{$data$}
    \Let{$a$}{$a + byte \bmod{65521}$}
    \Let{$b$}{$a + a \bmod{65521}$}
    \EndForEach
    \Return{$(b \ShiftLeft 16) \BitOr a$}
    \EndProcedure
  \end{algorithmic}
\end{algorithm}
