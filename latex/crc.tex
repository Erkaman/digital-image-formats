...
% This value is temporarily in this chapter
...
$\var{result}$ with the pattern $1111\ 1111$(=\hex{FF)}, you can make sure
...
\hex{FF}$. Many different values can be used instead of \hex{FF}. My
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Let{$a$}{$1$}
    \Let{$b$}{$0$}
    \ForEach{$\var{byte}$}{$\var{data}$}
    \Let{$a$}{$a + \var{byte} \bmod{65521}$}
