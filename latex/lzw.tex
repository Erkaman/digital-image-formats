...
table, numbers called \textit{codes}\index{code} are assigned to a
...
\Call{FirstNBits}{$n,bits$}\index{FirstNBits}. What this function
does, is that gets the value the first $bits$ least significant bits
of the number $n$. So the call \Call{FirstNBits}{$001101,3$}, for
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{OutputCode}{$codeSize$}
      ...
      \While{$remainingCodeBits > 0$} \Comment{If the packet will get filled in this packing.}
        \If{$remainingPacketBits \leq remainingCodeBits$}

          \linecomment{Read as many bytes necessary the fill up the packet.}
          \Let{$packet$}{$packet \BitOr ($
            \Call{FirstNBits{code,remainingPacketBits}} $\ShiftLeft
            shift)$}

          \linecomment{Write the packet}
          \Call{WriteByte}{$packet$}

          \linecomment{Throw away the bits of the code that has been
            packed in the now filled packet.}
        ...
        \Else \Comment{If the rest of the code fit in the entire
          packet.}

          \linecomment{Read in the entire code to the packet.}
          \Let{$packet$}{$packet \BitOr ($
            \Call{FirstNBits{code,remainingCodeBits}} $\ShiftLeft
            shift)$}
          ...
          \linecomment{The entire code has been processed, so
            terminate the loop}
          \Let{$remainingCodeBits$}{$0$}
        \EndIf
      \EndWhile
